...
完整核对项见 [RELEASE_CHECKLIST.zh-CN.md](RELEASE_CHECKLIST.zh-CN.md)。
...
- 核心保持零强制运行时依赖（见 [ADR-005](ADR.zh-CN.md#adr-005-python-stdlib-优先与零强制运行时依赖)）。  
...
- README / ARCHITECTURE 中的 CLI 命令  
- `.env.example` 中的 `NOVEL_WEAVER_*` 环境变量  
- workspace SQLite Canonical 结构（迁移时会说明）  

内部模块可能无预告变更。正式稳定性承诺文档待公开 Python 契约导出后再补。
